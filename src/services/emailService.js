...
// สร้าง HTML template สำหรับอีเมลคำสั่งซื้อ (ใช้ร่วมกัน)
const createOrderEmailHTML = (order, orderWithUser, showFooter = false) => {
  return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
            ...
            <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #0A0A0A; border-radius: 24px; overflow: hidden; border: 1px solid #7c4d33; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.5);">
              ...
              <tr>
                <td style="padding: 20px 30px; background-color: #1a1a1a; border-top: 1px solid #7c4d33; text-align: center;">
                  <p style="margin: 0 0 10px 0; color: #F5F1E6; font-size: 16px; font-weight: 500;">Thank you for shopping with us!</p>
                  <p style="margin: 0; color: #e3dcd4; font-size: 14px;">Your order has been successfully received.</p>
                </td>
              </tr>
              ` : ''}
...
// สร้าง text content สำหรับอีเมล (ใช้ร่วมกัน)
const createOrderEmailText = (order, orderWithUser) => {
...
// ส่งอีเมลไปหาลูกค้า (ใช้ template เดียวกัน)
const sendOrderConfirmationToCustomer = async (order) => {
  try {
    ...
    if (!orderWithUser.user.email) {
      console.log('ลูกค้าไม่มีอีเมล ข้ามการส่งอีเมล');
      return;
    }
    ...
    const mailOptions = {
      ...
      subject: `Order Confirmation #${order._id.toString()} - ${orderWithUser.user.name || 'Customer'}`,
      text: createOrderEmailText(order, orderWithUser),
      html: createOrderEmailHTML(order, orderWithUser, true) // true = แสดง footer สำหรับลูกค้า
    };
    
    const info = await transporter.sendMail(mailOptions);
    console.log(`อีเมลยืนยันคำสั่งซื้อส่งไปยังลูกค้า ${orderWithUser.user.email} แล้ว: ${info.messageId}`);
    
    return info;
  } catch (error) {
    console.error('เกิดข้อผิดพลาดในการส่งอีเมลไปหาลูกค้า:', error);
